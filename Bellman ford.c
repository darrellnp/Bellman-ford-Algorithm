...
{
    struct Graph* graph = (struct Graph*) malloc( sizeof(struct Graph)); //salah G nya ga gede kurang kurung 1 //2 3
//Allocating space to structure graph
    ...
    graph->edge = (struct Edge*) malloc( graph->E * sizeof( struct Edge ) );//salah > dan Edge kurang * //4 5
//Creating "Edge" type structures inside "Graph" structure, the number of edge type structures are equal to number of edges

    return;
}

void FinalSolution(int dist[], int n) //kurang []// 6
{
...
void BellmanFord(struct Graph* graph, int source)
{
    ...
    int E = graph->E; //change to E //9

    int StoreDistance[V]; // () salah hrs [] //10
...
// We assign source distance as 0(zero)
    ...
    for (i = 1; i <= V-1; i++)
    {
        for (j = 0; j < E; j++)// hrsnya ++ //12
        {
            ...
            if (StoreDistance[u] + weight < StoreDistance[v])
                StoreDistance[v] = StoreDistance[u] + weight; // =  1 aja //13
        }
    }
    ...
    for (i = 0; i < E; i++)
    {
        int u = graph->edge[i].source; // start not declared must be source //14
    ...
    FinalSolution(StoreDistance, V); // f kecil ganti F gede //15
...
printf("Enter your source vertex number\n");
    scanf("%d",&S); // printf diganti scanf //16
    ...
    BellmanFord(graph, S);// kebalik //17
